...
const HERO_SLIDES = [
  {
    ...
    cta: 'Journal',
    to: '/journal/release_worry',
    emoji: '🪶',
  },
  {
    id: 'growth',
    text: 'Discover where you are on your growth journey.',
    cta: 'Assessment',
    to: '/growth',
    emoji: '🌱',
  },
  {
    id: 'tests',
    text: 'Understand yourself with guided self-discovery tests.',
    cta: 'Explore',
    to: '/tests',
    emoji: '🧠',
  },
...
export default function Home() {
  ...
  const featuredMusic = suggestions?.music?.[0];
  const featuredActivity = suggestions?.activities?.[0];
  ...
  const displayQuote = quote || "I don't need to wait to rest — I deserve care now.";

  return (
    <div className="page home-page">
      <PageGreeting
        name={user?.display_name || user?.username}
        subtitle="Your space for calm, reflection, and growth."
      />

      <HeroCarousel slides={HERO_SLIDES} />

      <h2 className="section-eyebrow">Recommendations</h2>
      <div className="rec-grid">
        <Link to="/journal/calm_anxiety" className="rec-card rec-card--tall">
          <span className="rec-label">Sound</span>
          <span className="rec-title">
            {featuredMusic?.title || 'Calm focus playlist'}
          </span>
          <span className="rec-desc">
            {featuredMusic?.reason || 'Gentle audio to ease your mind and settle into the present moment.'}
          </span>
          <div className="rec-footer">
            <span className="rec-meta">{featuredMusic?.duration || '5 min'}</span>
            <span className="rec-play" aria-hidden="true">▶</span>
          </div>
        </Link>

        <Link to="/growth" className="rec-card">
          <span className="rec-icon-wrap" aria-hidden="true">🌙</span>
          <span className="rec-label">Growth</span>
          <span className="rec-title">
            {featuredActivity?.title || 'Mindful check-in'}
          </span>
        </Link>

        <div className="rec-card">
          <span className="rec-label">Quote</span>
          <p className="rec-quote">{displayQuote}</p>
        </div>

        <Link to="/tests" className="rec-card">
          <span className="rec-label">Discover</span>
          <span className="rec-title">Self-discovery tests</span>
          <span className="rec-art" aria-hidden="true">🌈</span>
        </Link>
      </div>

      <section className="home-section">
        <h2 className="home-section__title">Daily routine</h2>
          ...
          <div className="tasknb-wrap">
            ...
            <section className="tasknb-book">
              <div className="tasknb-book-header">
                <div className="tasknb-book-title">Today&apos;s tasks</div>
                <div className="tasknb-book-meta">tap status · add notes</div>
              </div>
            ...
            {quote && (
              <section className="tasknb-quote-card">
                <div className="tasknb-quote-mark">&ldquo;</div>
            ...
            {suggestions?.insight && (
              <section className="tasknb-insight-card home-insight" style={{ background: suggestions.insight.color }}>
