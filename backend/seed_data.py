from models import db, TestDefinition, TestQuestion, JournalPrompt, AssessmentQuestion


def seed_all():
    """Seed the database with required reference data without duplicating rows."""

    def ensure_row(model, lookup, defaults=None):
    ...
    for jtype, text, order in journal_prompts:
        ensure_row(
            JournalPrompt,
            {'journal_type': jtype, 'question_text': text, 'order_num': order},
        )
    ...
    for test_data in tests_data:
        ...
        existing_test = TestDefinition.query.filter_by(title=test_data['title']).first()
        ...
        existing_questions = {
            ...
            for question in TestQuestion.query.filter_by(test_id=test.id).all()
        }

        for i, q_text in enumerate(questions):
            question_key = (i + 1, q_text)
            if question_key not in existing_questions:
                db.session.add(TestQuestion(test_id=test.id, question_text=q_text, order_num=i + 1))

    # === ASSESSMENT QUESTIONS (Growth) ===
    ...
    for text, category, order in assessment_questions:
        ensure_row(
            AssessmentQuestion,
            {'question_text': text, 'category': category, 'order_num': order},
        )
